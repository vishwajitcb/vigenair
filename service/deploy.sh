...
gcloud config set project chai-shots-465914
gcloud services enable cloudresourcemanager.googleapis.com
gcloud auth application-default set-quota-project chai-shots-465914
printf "\nINFO - GCP project set to 'chai-shots-465914' succesfully!\n"

BUCKET_EXISTS=$(gcloud storage ls gs://chai-shots-465914-vigenair > /dev/null 2>&1 && echo "true" || echo "false")
if "${BUCKET_EXISTS}"; then
  printf "\nWARN - Bucket 'chai-shots-465914-vigenair' already exists. Skipping bucket creation...\n"
else
  gcloud storage buckets create gs://chai-shots-465914-vigenair --project=chai-shots-465914 --location=us --uniform-bucket-level-access
  test $? -eq 0 || exit
  printf "\nINFO - Bucket 'chai-shots-465914-vigenair' created successfully in location 'us'!\n"
fi
...
PROJECT_NUMBER=$(gcloud projects describe chai-shots-465914 --format="value(projectNumber)")
...
for SA in "aiplatform.googleapis.com" "storage.googleapis.com" "eventarc.googleapis.com"; do
    gcloud --no-user-output-enabled beta services identity create --project=chai-shots-465914 \
        --service="${SA}"
done
COMPUTE_SA_ROLES=(
...
--trigger-event-filters="bucket=chai-shots-465914-vigenair" \
--trigger-location="us"
test $? -eq 0 || exit
